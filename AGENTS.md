...
- 링크나 주제를 “정리해줘”라고 요청하면 채팅 요약 대신 학습 페이지를 만들고, 입력을 바꿔 실행할 수 있는 API 실습을 기본으로 포함합니다.
- 기존 API와 실습이 있으면 재사용합니다. 요청·응답·오류·로딩 상태와 실제 구현 파일·메서드의 브레이크포인트 위치를 안내합니다.
- JavaScript 이벤트 루프처럼 실행 환경이 중요한 주제는 브라우저에서 실행하고 실제 API와의 경계도 확인할 수 있게 합니다.
- 로컬 모형·모의 데이터·실제 외부 인프라를 구분합니다. 대표 실습의 범위와 생략한 기능을 화면에 명시하고, 설명 전체가 실행된다고 표현하지 않습니다.
- 실행 예제, 입력 제한, 실패 경로를 검증하고 실행 기능 누락 검사도 갱신합니다.
